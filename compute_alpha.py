...
def compute_alpha(omega, material):
    ...
    resolution = 50  # := number of elements along L
    ...
    def g_k(k):
        ...
        sigma = 1.7
        y0 = 5.0
        return G0 * numpy.sqrt(2 * numpy.pi) * sigma * numpy.exp(-1j * y0 * k) * numpy.exp(-0.5 * (sigma * k)**2)
...
def run_compute_alpha(material):
    ...
    omegas = numpy.linspace(2.0 * numpy.pi, numpy.pi * 1e9, num=numb_omega)
